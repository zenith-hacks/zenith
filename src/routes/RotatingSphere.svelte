...
<div class="outer-sphere" style={style}>
	<div class="sphere">
		<div class="sphere-outline"></div>
		{#each Array(verticalLines) as _, i}
			<div class="y right" style={`--w: ${Math.abs(Math.sin(i * verticalAngle) * 100)}%; --wt: ${Math.abs(Math.sin((i + 1) * verticalAngle) * 100)}%`}></div>
		{/each}
		{#each Array(verticalLines + 1) as _, i}
			{#if i == 0}
				<div class="y left" style={`--wt: ${Math.abs(Math.sin(i * verticalAngle) * 100)}%; --w: ${Math.abs(Math.sin((i - 1) * verticalAngle) * 100)}%`}></div>
			{:else}
				<div class="y left" style={`--w: ${Math.abs(Math.sin(i * verticalAngle) * 100)}%; --wt: ${Math.abs(Math.sin((i - 1) * verticalAngle) * 100)}%`}></div>
			{/if}
		{/each}
